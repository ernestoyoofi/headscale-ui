...
func API_LoginAccount(w http.ResponseWriter, r *http.Request) {
  w.Header().Set("Content-Type", "application/json")

  log.Println("[api/login-account]: [Request] /side-api/login")
  if r.Method != http.MethodPost {
    log.Println("[api/login-account]: [No Method] /side-api/login")
  ...
  // Database action!
  jwtToken, errs := service.Service_LoginAccount(dataReqJson.Username, dataReqJson.Password)
  if errs != "" {
